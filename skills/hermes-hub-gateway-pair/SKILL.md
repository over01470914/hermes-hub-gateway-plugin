...
by the pairing client. Do not infer expiry from a calendar date or chat
timestamp alone: use the complete RFC3339 UTC timestamp, then let the CLI
validate the current Router pairing record authoritatively.

Before step 1, find `Expires (UTC, RFC3339)` in the Router pairing request and
compare it with the host's current UTC time. If fewer than 180 seconds remain,
do not install, restart, or pair; request a fresh Router pairing request. If
the threshold is met, continue. Compare the complete RFC3339 timestamps, not
only their calendar dates. The CLI repeats this as the final authoritative
expiry check immediately before mutation.
